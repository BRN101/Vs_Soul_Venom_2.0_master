...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'sighofdespair':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/sighofdespair/sighofdespairDialogue'));
			case 'getout':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/getout/getoutDialogue'));
			case 'soulvenom':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/soulvenom/soulvenomDialogue'));
		}
				...
				case 'alley': 
					{
						...
						var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('alley/sky'));
						bg.scrollFactor.set(0.1, 0.1);
						add(bg);
		
						var city:FlxSprite = new FlxSprite(-10).loadGraphic(Paths.image('alley/city'));
						city.scrollFactor.set(0.3, 0.3);
						city.setGraphicSize(Std.int(city.width * 0.85));
						city.updateHitbox();
						add(city);
						...
						for (i in 0...5)
						{
								var light:FlxSprite = new FlxSprite(city.x).loadGraphic(Paths.image('alley/win' + i));
								light.scrollFactor.set(0.3, 0.3);
								light.visible = false;
								light.setGraphicSize(Std.int(light.width * 0.85));
								light.updateHitbox();
						...
						var streetBehind:FlxSprite = new FlxSprite(-40, 50).loadGraphic(Paths.image('alley/behindTrain'));
						add(streetBehind);
		
						phillyTrain = new FlxSprite(2000, 360).loadGraphic(Paths.image('alley/train'));
						if(FlxG.save.data.distractions){
							add(phillyTrain);
						}
		
						trainSound = new FlxSound().loadEmbedded(Paths.sound('train_passes2','week3'));
						FlxG.sound.list.add(trainSound);
		
						// var cityLights:FlxSprite = new FlxSprite().loadGraphic(AssetPaths.win0.png);
		
						var street:FlxSprite = new FlxSprite(-40, streetBehind.y).loadGraphic(Paths.image('alley/street'));
						add(street);

						if (songLowercase == 'soulvenom')
							{
							 health = 100;
							}
					}
				case 'minus':
					{
						defaultCamZoom = 0.74;
						curStage = 'minus';
						var bg:FlxSprite = new FlxSprite(-600, -270).loadGraphic(Paths.image('minus/stageback'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var bgacid:FlxSprite = new FlxSprite(-600, -270);
						bgacid.frames = Paths.getSparrowAtlas('minus/acid');
						bgacid.animation.addByPrefix('idle', 'acid', 24);
						bgacid.animation.play('idle');
						bgacid.scrollFactor.set(0.9, 0.9);
						add(bgacid);

						var bgbook:FlxSprite = new FlxSprite(-600, -270);
						bgbook.frames = Paths.getSparrowAtlas('minus/book');
						bgbook.animation.addByPrefix('idle', 'book', 24);
						bgbook.animation.play('idle');
						bgbook.scrollFactor.set(0.9, 0.9);
						add(bgbook);

						var stageCurtains:FlxSprite = new FlxSprite(-600, -270).loadGraphic(Paths.image('minus/stagecurtains'));
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					switch (Math.abs(daNote.noteData))
					{
						case 2:
							...
							if(curSong.toLowerCase() == 'soulvdila'){
							gf.playAnim('scared', true);
							FlxG.camera.shake(0.02,0.05);
							camHUD.shake(0.03,0.01);
							}
							else if(curSong.toLowerCase() == 'soulvenom'){
							health -= 0.02;
							}
						case 3:
